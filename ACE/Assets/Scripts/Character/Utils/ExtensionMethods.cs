...
public static class ExtensionMethods
{
    ...
    public static Rigidbody2D GetMainRigidbody2D(this GameObject g)
    {
        foreach (Rigidbody2D r in g.GetComponentsInChildren<Rigidbody2D>())
        {
            if (r.CompareTag("MainBody")) return r;
        }
        return (g.GetComponent<Rigidbody2D>()) ? g.GetComponent<Rigidbody2D>() : g.GetComponentInChildren<Rigidbody2D>();
    }
